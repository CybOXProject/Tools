...
#
# Generated Tue Feb 07 14:29:40 2012 by generateDS.py version 2.7b.
#
...
from common_types_1_0 import StringObjectAttributeType as Address_Value
from common_types_1_0 import StringObjectAttributeType as Ext_Category
from common_types_1_0 import StringObjectAttributeType as VLAN_Name
from common_types_1_0 import IntegerObjectAttributeType as VLAN_Num
...
class AddressObjectType(common.DefinedObjectType):
    """The AddressObjectType is intended to characterize IP, email, MAC,
    ATM, and ASN addresses. It is based on the IODEF address
    element.The category attribute specifies the address category
    that is being defined."""
    subclass = None
    superclass = common.DefinedObjectType
    def __init__(self, category='ipv4-addr', Address_Value=None, Ext_Category=None, VLAN_Name=None, VLAN_Num=None):
        super(AddressObjectType, self).__init__(None)
    ...
    def export(self, outfile, level, namespace_='AddressObj:', name_='AddressObjectType', namespacedef_=''):
        showIndent(outfile, level)
    ...
    def exportChildren(self, outfile, level, namespace_='AddressObj:', name_='AddressObjectType', fromsubclass_=False):
        super(AddressObjectType, self).exportChildren(outfile, level, namespace_, name_, True)
        if self.Address_Value is not None:
            self.Address_Value.export(outfile, level, 'AddressObj:', name_='Address_Value')
        if self.Ext_Category is not None:
            self.Ext_Category.export(outfile, level, 'AddressObj:', name_='Ext_Category')
        if self.VLAN_Name is not None:
            self.VLAN_Name.export(outfile, level, 'AddressObj:', name_='VLAN_Name')
        if self.VLAN_Num is not None:
            self.VLAN_Num.export(outfile, level, 'AddressObj:', name_='VLAN_Num')
    def hasContent_(self):
        if (
            self.Address_Value is not None or
            self.Ext_Category is not None or
            self.VLAN_Name is not None or
            self.VLAN_Num is not None
            ):
            return True
        else:
            return False
    ...
    def exportLiteralChildren(self, outfile, level, name_):
        ...
        if self.VLAN_Num is not None:
            showIndent(outfile, level)
            outfile.write('VLAN_Num=%s,\n' % quote_python(self.VLAN_Num).encode(ExternalEncoding))
    def build(self, node):
...
def parseLiteral(inFileName):
    ...
    # Enable Python to collect the space used by the DOM.
    doc = None
    sys.stdout.write('#from address_object import *\n\n')
    sys.stdout.write('import address_object as model_\n\n')
